...
function validate(elem, pattern) {
    var res = pattern.test(elem.getAttribute('value'));
    if (res === false) {
...
function onsubmitHandler(e) {
    ...
    for (var i = 0; i < form1.elements.length; ++i) {
        if (form1.elements[i].getAttribute('type') === 'text')
            form1.elements[i].setAttribute('class', 'valid');
    }
    ...
    for (var i = 0; i < form1.elements.length; ++i) {
        ...
        if (eL.getAttribute('type') == 'text' && eL.onchange) {
            eL.onchange(); // запуск события onchanhe
            if (eL.getAttribute('class') == 'invalid') 
            invalid = true;
        }
